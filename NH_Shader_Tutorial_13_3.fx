...
uniform int gKernelSize <
    ...
    //string UIFieldNames = "5x5:7x7:9x9:11x11:13x13:15x15:17x17:19x19:21x21:23x23:25x25:27x27:29x29:31x31:33x33:35x35";
    int UIMin = 3;
    int UIMax = 35;
    int UIStep = 2.0f;
> = 5.0f;
...
float CalcGaus(float2 UV){
    ...
    float2 offsetU = float2(texelSize, 0.0f);
    float2 offsetV = float2(0.0f, texelSize);
    float2 coordU = UV - distance * offsetU;
    float2 coordV = UV - distance * offsetV;
    float output = 0.0f;
    for(int i = 0; i < gKernelSize; i++){
        output += gLight0ShadowMap.Sample(gWrapSampler, coordU).r * weight[i] * 0.5;
        coordU += offsetU;
    }
    for(int i = 0; i < gKernelSize; i++){
