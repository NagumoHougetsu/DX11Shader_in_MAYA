...
float2 CalcGaus(float2 UV){
    ...
    if(gSamplingPoints == 0){
        for(int i = 0; i < gKernelSize; i++){
            output.x += CalcDepth(coord_01, gLight0ShadowMap) * weight[i] * 0.5;
            coord_01 -= offset_01;
            output.x += CalcDepth(coord_03, gLight0ShadowMap) * weight[i] * 0.5;
            coord_03 -= offset_03;
            /*
        ...
        for(int i = 0; i < gKernelSize; i++){
            output.x += CalcDepth(coord_01, gLight0ShadowMap) * weight[i] * 0.25;
            coord_01 -= offset_01;
            output.x += CalcDepth(coord_02, gLight0ShadowMap) * weight[i] * 0.25;
            coord_02 -= offset_02;
            output.x += CalcDepth(coord_03, gLight0ShadowMap) * weight[i] * 0.25;
            coord_03 -= offset_03;
            output.x += CalcDepth(coord_04, gLight0ShadowMap) * weight[i] * 0.25;
            coord_04 -= offset_04;
            /*
        ...
        if(gShadowMethod == 0){
            ...
            if(shadowMapUV.x > 0.0f && shadowMapUV.x < 1.0f && shadowMapUV.y > 0.0f && shadowMapUV.y < 1.0f){
                ...
                if(zInLVP > shadowValue.x){
                    ...
                    float variance = clamp(0.0001f, 1.0f, shadowValue.y * shadowValue.y - shadowValue.x * shadowValue.x);
