...
uniform int gKernelSize <
    ...
    string UIFieldNames = "3x3:5x5:7x7:9x9";
    int UIMin = 3;
    int UIMax = 9;
    int UIStep = 2;
> = 3;
...
float CulcWeight(float sigma, float x){
...
float CalcGaus(float2 UV){
    ...
    int halfSample = gKernelSize / 2.0f;
    float pixelSize = 1.0 / size;
    for(int iy = 0; iy < halfSample; iy++){
        float deltaY = float(iy - halfSample);
        float fy = CulcWeight(gSD, deltaY);
        float offsetY = deltaY * pixelSize;
        for(int ix = 0; ix < halfSample; ix++){
            float deltaX = float(ix - halfSample);
            float fx = CulcWeight(gSD, deltaX);
            float offsetX = deltaX * pixelSize;
            float weight = fx * fy;
            mapOutput = gLight0ShadowMap.SampleLevel(gWrapSampler, UV + float2(offsetX, offsetY), 0).r * weight;
            weightSum += weight;
        }
    }
    return mapOutput / weightSum;
}
