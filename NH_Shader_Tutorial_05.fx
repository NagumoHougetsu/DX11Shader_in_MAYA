...
float CulcShade(float4 Normal){
    float Out;
    float3 lightDir = normalize(gLight0Dir);
    Out = dot(Normal.xyz, -lightDir);
    Out = smoothstep(gToonThreshold - gToonFeather, gToonThreshold + gToonFeather, Out);
    return Out;
}

float4 PS(VS_TO_PS In) : SV_Target{
    float N = CulcShade(In.Normal);
    float3 color;
    float3 baseColor;
    float3 shadowColor;
    if(gUseBaseColorTexture == true){
        baseColor = gBaseColorTexture.Sample(gWrapSampler, In.UV).xyz;
    }else{
        baseColor = gBaseColor;
    }
    if(gUseShadowColorTexture == true){
        shadowColor = gShadowColorTexture.Sample(gWrapSampler, In.UV).xyz;
    }else{
...
float4 PS_OUTLINE(VS_TO_PS In) : SV_Target{
    ...
    if(gUseColorTresses == true){
        ...
        float3 tressColor = lerp(shadowColor, baseColor, N);
        color *= tressColor;
    }
    return float4(color, 1.0);
}
